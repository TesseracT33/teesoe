...
static bool dma_in_progress, dma_is_pending;
/* What was written last to either SP_DMA_RDLEN/ SP_DMA_WRLEN during an ongoing DMA */
static s32 buffered_dma_rdlen, buffered_dma_wrlen, dma_spaddr_last_addr, dma_ramaddr_last_addr;
static DmaType in_progress_dma_type;
static void (*init_pending_dma_fun_ptr)();
...
template<DmaType dma_type> void InitDMA()
{
    ...
    s32 bytes_to_copy = std::min(requested_total_bytes, std::min(num_bytes_until_rdram_end, num_bytes_until_spram_end));
    ...
    instance to transfer a rectangular portion of a larger image, by specifying the size of each row of the selection
