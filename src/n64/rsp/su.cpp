...
template<ScalarInstruction instr> void ScalarLoad(u32 instr_code)
{
    ...
    if constexpr (log_rsp_instructions) {
        current_instr_log_output = std::format("{} {}, ${:X}",
          current_instr_name,
          rt,
          static_cast<std::make_unsigned<decltype(address)>::type>(address));
    }
    ...
    auto result = [&] {
        if constexpr (instr == LB) {
            ...
            static_assert(always_false<instr>,
              "\"Load\" template function called, but no matching load ScalarInstruction was found.");
        }
    }();
...
template<ScalarInstruction instr> void ScalarStore(u32 instr_code)
{
    ...
    if constexpr (instr == SB) {
        ...
        static_assert(always_false<instr>,
          "\"Store\" template function called, but no matching store ScalarInstruction was found.");
    }
...
template<ScalarInstruction instr> void AluImmediate(u32 instr_code)
{
    ...
    auto immediate = [&] {
        if constexpr (one_of(instr, ANDI, LUI, ORI, XORI)) return u16(instr_code & 0xFFFF);
        else return s16(instr_code & 0xFFFF);
    }();

    if constexpr (log_rsp_instructions) {
        current_instr_log_output = [&] {
            if constexpr (instr == LUI) return std::format("{} {}, ${:X}", current_instr_name, rt, immediate);
            else return std::format("{} {}, {}, ${:X}", current_instr_name, rt, rs, immediate);
        }();
    }
    ...
    gpr.set(rt, [&] {
        if constexpr (instr == ADDI || instr == ADDIU) {
            ...
            static_assert(always_false<instr>,
              "\"ALU_Immediate\" template function called, but no matching ALU immediate ScalarInstruction was found.");
        }
    }());
...
template<ScalarInstruction instr> void AluThreeOperand(u32 instr_code)
{
    ...
    gpr.set(rd, [&] {
        if constexpr (instr == ADD || instr == ADDU) {
            ...
            static_assert(always_false<instr>,
              "\"ALU_ThreeOperand\" template function called, but no matching ALU three-operand ScalarInstruction was "
              "found.");
        }
    }());
...
template<ScalarInstruction instr> void Shift(u32 instr_code)
{
    using enum ScalarInstruction;
    ...
    if constexpr (log_rsp_instructions) {
        current_instr_log_output = [&] {
            if (instr_code == 0) return std::string("NOP");
            if constexpr (one_of(instr, SLLV, SRLV, SRAV))
                return std::format("{} {}, {}, {}", current_instr_name, rd, rt, rs);
            else return std::format("{} {}, {}, {}", current_instr_name, rd, rt, sa);
        }();
    }
    ...
    gpr.set(rd, [&] {
        if constexpr (instr == SLL) {
            ...
            static_assert(always_false<instr>,
              "\"ALU_Shift\" template function called, but no matching ALU shift ScalarInstruction was found.");
        }
    }());
    ...
    if (!in_branch_delay_slot) {
        u32 target = [&] {
            if constexpr (instr == J || instr == JAL) {
                ...
                static_assert(always_false<instr>,
                  "\"Jump\" template function called, but no matching jump ScalarInstruction was found.");
            }
        }();
        PrepareJump(target);
    }
...
template<ScalarInstruction instr> void Branch(u32 instr_code)
{
    ...
    if constexpr (log_rsp_instructions) {
        current_instr_log_output = [&] {
            ...
            else return std::format("{} {}, ${:X}", current_instr_name, rs, offset);
        }();
    }

    /* For all ScalarInstructions: branch to the branch address if the condition is met, with a delay of one
       ScalarInstruction. For "link" ScalarInstructions: stores the address of the ScalarInstruction following the delay
       slot to register r31 (link register). */
...
template<ScalarInstruction instr> void Move(u32 instr_code)
{
    using enum ScalarInstruction;
    ...
    if constexpr (instr == MFC0) {
        ...
        static_assert(always_false<instr>,
          "\"RSP Move\" template function called, but no matching move instruction was found.");
    }
