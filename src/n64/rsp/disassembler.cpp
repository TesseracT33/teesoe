...
#define EXEC_SCALAR_INSTR(INSTR)                                         \
    {                                                                    \
        if constexpr (log_rsp_instructions) current_instr_name = #INSTR; \
...
template<ScalarInstruction instr> void ExecuteScalarInstruction()
{
    ...
    if constexpr (one_of(instr, LB, LBU, LH, LHU, LW, LWU, LL)) {
        ScalarLoad<instr>(instr_code);
    } else if constexpr (one_of(instr, SB, SH, SW, SC)) {
        ScalarStore<instr>(instr_code);
    } else if constexpr (one_of(instr, ADDI, ADDIU, SLTI, SLTIU, ANDI, ORI, XORI, LUI)) {
        AluImmediate<instr>(instr_code);
    } else if constexpr (one_of(instr, ADD, ADDU, AND, NOR, OR, SLT, SLTU, SUB, SUBU, XOR)) {
        AluThreeOperand<instr>(instr_code);
    } else if constexpr (one_of(instr, SLL, SRL, SRA, SLLV, SRLV, SRAV)) {
        Shift<instr>(instr_code);
    } else if constexpr (one_of(instr, J, JAL, JR, JALR)) {
        Jump<instr>(instr_code);
    } else if constexpr (one_of(instr, BEQ, BNE, BLEZ, BGTZ, BLTZ, BGEZ, BLTZAL, BGEZAL)) {
        Branch<instr>(instr_code);
    } else if constexpr (one_of(instr, MFC0, MTC0)) {
...
template<VectorInstruction instr> void ExecuteVectorInstruction()
{
    ...
    if constexpr (one_of(instr, LBV, LSV, LLV, LDV, LQV, LRV, LPV, LUV, LTV, LHV, LFV, LWV)) {
        VectorLoadStore<instr>(instr_code);
    } else if constexpr (one_of(instr, SBV, SSV, SLV, SDV, SQV, SRV, SPV, SUV, STV, SHV, SFV, SWV)) {
        VectorLoadStore<instr>(instr_code);
    } else if constexpr (one_of(instr, MTC2, MFC2, CTC2, CFC2)) {
        Move<instr>(instr_code);
    } else if constexpr (one_of(instr, VMOV, VRCP, VRSQ, VRCPH, VRSQH, VRCPL, VRSQL, VRNDN, VRNDP, VNOP)) {
    ...
    } else if constexpr (one_of(instr, VLT, VEQ, VNE, VGE, VCH, VCR, VCL, VMRG)) {
