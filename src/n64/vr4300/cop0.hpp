...
struct Cop0Registers {
    ...
    u64 count; /* (9); Increments every other PClock. When equal to the Compare register, interrupt bit IP(7) in the
                  Cause register is set. */
    /* On real HW, the register is 32 bits. Here, we make it 64 bits and increment it every PCycle instead of every
    other PCycle. When we read from it, we shift it right one bit and then return it. When we write to it, we set it to
    the data shifted left one bit. */
    ...
    u64 compare; /* (11); When equal to the Count register, interrupt bit IP(7) in the Cause register is set. Writes to
    ...
    struct { /* (28) */
        u32        : 6;
        u32 pstate : 2; /* Specified the primary cache state (Data cache: 11 => valid; 00 => invalid) (Instruction
                           cache: 10 => valid; 00 => invalid). All others: undefined. */
...
public:
    u32 Generate() { return distrib(gen); }
    void SetRange(auto wired)
    { /* If WIRED > 31, RANDOM can be [0, 63] */
        if (wired <= 31) distrib = std::uniform_int_distribution<u32>{ wired, 31 };
        else distrib = std::uniform_int_distribution<u32>{ 0, 63 };
    }
