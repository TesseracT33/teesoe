    ...
    /* Unaligned memory access example: demonstrating that the below works.
       ...
       The end result should be: reg = 1234
        ...
        4567		7654		after read
                    4567		after swap
                    XXX4		after shift (>> 24); 24 == 8 * (sizeof word - 1 - (addr & (sizeof word - 1))) == 8 * (3
       - (4 & 3))
...
template<CpuInstruction instr> void Store(u32 rs, u32 rt, s16 imm16)
{
    using enum CpuInstruction;
    ...
    /* For all instructions:
       ...
       virtual address creation. */
    if constexpr (one_of(instr, SD, SDL, SDR, SCD)) {
        if (operating_mode == OperatingMode::Kernel && addressing_mode == AddressingMode::_32bit) {
            addr = s32(addr);
        }
    }
...
template<CpuInstruction instr> void AluImmediate(u32 rs, u32 rt, s16 imm16)
{
    using enum CpuInstruction;

    auto imm = [&] {
        if constexpr (one_of(instr, ANDI, LUI, ORI, XORI)) return u16(imm16);
...
template<CpuInstruction instr> void AluThreeOperand(u32 rs, u32 rt, u32 rd)
{
    using enum CpuInstruction;
...
template<CpuInstruction instr> void Shift(u32 rt, u32 rd, u32 sa)
{
    using enum CpuInstruction;
...
template<CpuInstruction instr> void ShiftVariable(u32 rs, u32 rt, u32 rd)
{
    using enum CpuInstruction;
...
template<CpuInstruction instr> void MulDiv(u32 rs, u32 rt)
{
    using enum CpuInstruction;
    ...
    static constexpr int cycles = [&] {
        if constexpr (one_of(instr, MULT, MULTU)) return 5;
        else if constexpr (one_of(instr, DMULT, DMULTU)) return 8;
        else if constexpr (one_of(instr, DIV, DIVU)) return 37;
        else if constexpr (one_of(instr, DDIV, DDIVU)) return 69;
        else static_assert(always_false<instr>);
    }();
    AdvancePipeline(cycles);
}

template<CpuInstruction instr> void Branch(u32 rs, s16 imm16)
{
    using enum CpuInstruction;
    ...
    bool branch_cond = [&] {
        if constexpr (one_of(instr, BLEZ, BLEZL)) return gpr[rs] <= 0;
        else if constexpr (one_of(instr, BGTZ, BGTZL)) return gpr[rs] > 0;
        else if constexpr (one_of(instr, BLTZ, BLTZL)) return gpr[rs] < 0;
        else if constexpr (one_of(instr, BGEZ, BGEZL)) return gpr[rs] >= 0;
        else if constexpr (one_of(instr, BLTZAL, BLTZALL)) return gpr[rs] < 0;
        else if constexpr (one_of(instr, BGEZAL, BGEZALL)) return gpr[rs] >= 0;
        else static_assert(always_false<instr>);
    }();
    ...
    } else if constexpr (one_of(instr, BLEZL, BGTZL, BEQL, BLTZL, BGEZL, BLTZALL, BGEZALL)) {
...
template<CpuInstruction instr> void Branch(u32 rs, u32 rt, s16 imm16)
{
    using enum CpuInstruction;

    bool branch_cond = [&] {
        if constexpr (one_of(instr, BEQ, BEQL)) return gpr[rs] == gpr[rt];
        else if constexpr (one_of(instr, BNE, BNEL)) return gpr[rs] != gpr[rt];
        else static_assert(always_false<instr>);
    }();
    if (branch_cond) {
        auto offset = imm16 << 2;
        PrepareJump(pc + offset);
    } else if constexpr (one_of(instr, BEQL, BNEL)) {
        pc += 4; /* The instruction in the branch delay slot is discarded. */
        AdvancePipeline(2); // COUNT is increased for the branch delay instruction even if it is not executed
        return;
    }
...
template<CpuInstruction instr> void TrapThreeOperand(u32 rs, u32 rt)
{
    using enum CpuInstruction;
...
template<CpuInstruction instr> void TrapImmediate(u32 rs, s16 imm16)
{
    using enum CpuInstruction;
