...
inline void dmfc0(u32 rt, u32 rd)
{
    ...
    // if (!rt) return;
    // Gpq ht = get_gpr_and_mark_dirty(rt);

    // auto Read = [ht](auto const& reg) {
    //     if constexpr (sizeof(reg) == 4) c.mov(ht.r32(), ptr(reg));
    //     else if constexpr (sizeof(reg) == 8) c.mov(ht.r64(), ptr(reg));
    //     else static_assert(always_false<sizeof(reg)>, "Register must be either 4 or 8 bytes.");
    // };

    // switch (rd & 31) {
    // case Cop0Reg::index: Read(cop0.index); break;
    // case Cop0Reg::random: random_generator.Generate(); break; /* Generate a random number in the interval [wired, 31]
    // */ case Cop0Reg::entry_lo_0: Read(cop0.entry_lo[0]); break; case Cop0Reg::entry_lo_1: Read(cop0.entry_lo[1]);
    // break; case Cop0Reg::context: Read(cop0.context); break; case Cop0Reg::page_mask: Read(cop0.page_mask); break;
    // case Cop0Reg::wired: Read(cop0.wired); break;
    // case Cop0Reg::bad_v_addr: Read(cop0.bad_v_addr); break;
    // case Cop0Reg::count: u32(count >> 1); break; // TODO: need to have a "jit_count"
...
inline void mfc0(u32 rt, u32 rd)
{
    ...
    // auto Read = [ht](auto const& reg) {
    //     if constexpr (sizeof(reg) == 4) c.mov(ht.r32(), dword_ptr(reg));
    //     else if constexpr (sizeof(reg) == 8) c.movsxd(ht.r64(), dword_ptr(reg));
    //     else static_assert(always_false<sizeof(reg)>, "Register must be either 4 or 8 bytes.");
    // };
