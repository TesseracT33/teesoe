...
#define EXEC_CPU_INSTR(INSTR)                                            \
    {                                                                    \
        if constexpr (log_cpu_instructions) current_instr_name = #INSTR; \
...
void DecodeExecuteCop0Instruction()
{
    ...
    default: NotifyIllegalInstrCode(instr_code); SignalException<Exception::ReservedInstruction>();
    }
}
...
void DecodeExecuteCop2Instruction()
{
    ...
    default:
        cop0.status.cu2 ? SignalException<Exception::ReservedInstructionCop2>() : SignalCoprocessorUnusableException(2);
        AdvancePipeline(1);
    }
}
...
void DecodeExecuteCOP3Instruction()
{
    ...
    if (opcode == 0) { /* MFC3 */
        ...
        cop0.status.cu3 ? SignalException<Exception::ReservedInstruction>() : SignalCoprocessorUnusableException(3);
    }
    AdvancePipeline(1);
}
...
template<CpuInstruction instr> void ExecuteCpuInstruction()
{
    ...
    if constexpr (instr == CpuInstruction::J) {
        ...
        } else if constexpr (one_of(instr,
                               ADD,
                               ADDU,
                               AND,
                               DADD,
        ...
        } else if constexpr (one_of(instr, SLL, SRL, SRA, DSLL, DSRL, DSRA, DSLL32, DSRL32, DSRA32)) {
            if (instr_code == 0) {
                LOG_INSTR("NOP");
            } else {
        ...
        } else if constexpr (one_of(instr, SLLV, SRLV, SRAV, DSLLV, DSRLV, DSRAV)) {
            LOG_INSTR(std::format("{} {}, {}, {}", current_instr_name, RD, RT, RS));
            ShiftVariable<instr>(RS, RT, RD);
        } else if constexpr (one_of(instr, MULT, MULTU, DIV, DIVU, DMULT, DMULTU, DDIV, DDIVU)) {
        ...
        } else if constexpr (one_of(instr, BEQ, BEQL, BNE, BNEL)) {
            LOG_INSTR(std::format("{} {}, {}, ${:X}", current_instr_name, RS, RT, IMM16));
            Branch<instr>(RS, RT, IMM16);
        } else if constexpr (one_of(instr, TGE, TGEU, TLT, TLTU, TEQ, TNE)) {
            LOG_INSTR(std::format("{} {}, {}", current_instr_name, RS, RT));
            TrapThreeOperand<instr>(RS, RT);
        } else if constexpr (one_of(instr, TGEI, TGEIU, TLTI, TLTIU, TEQI, TNEI)) {
            LOG_INSTR(std::format("{} {}, ${:X}", current_instr_name, RS, IMM16));
...
{
    if constexpr (one_of(instr,
                    Cop0Instruction::MTC0,
...
template<Cop1Instruction instr> void ExecuteCop1Instruction()
{
    ...
    if constexpr (one_of(instr, LWC1, LDC1)) {
        FpuLoad<instr>(instr_code);
    } else if constexpr (one_of(instr, SWC1, SDC1)) {
        FpuStore<instr>(instr_code);
    } else if constexpr (one_of(instr, MTC1, MFC1, CTC1, CFC1, DMTC1, DMFC1, DCFC1, DCTC1)) {
    ...
    } else if constexpr (one_of(instr, ADD, SUB, MUL, DIV, ABS, MOV, NEG, SQRT)) {
        FpuCompute<instr>(instr_code);
    } else if constexpr (one_of(instr, BC1T, BC1F, BC1TL, BC1FL)) {
...
template<Cop2Instruction instr> void ExecuteCop2Instruction()
{
    ...
    if constexpr (one_of(instr, CFC2, CTC2, MFC2, MTC2, DCFC2, DCTC2, DMFC2, DMTC2)) {
        Cop2Move<instr>(RT);
    } else {
        static_assert(always_false<instr>);
    }
